...
export const FinancialReports: React.FC = () => {
  ...
  const renderOverview = () => {
    ...
    const cards = [
      ...
      { label: '总购买价值', value: formatCurrency(s.totalPurchasePrice), sub: `SPA ${formatCurrency(s.totalSpaPrice)}${s.totalSpaPrice !== s.totalPurchasePrice ? ` · 台底 ${formatCurrency(s.totalSpaPrice - s.totalPurchasePrice)}` : ''} · 其他 ${formatCurrency(s.totalPurchaseFees)}`, icon: <Building2 size={20} />, bg: 'bg-accent/15', color: 'text-accent' },
      ...
      ...(s.totalMarketValue > 0 ? [(() => { const diff = s.totalMarketValue - s.totalPurchaseValue; const pct = s.totalPurchaseValue > 0 ? (diff / s.totalPurchaseValue * 100).toFixed(1) : '0'; return { label: '总市值', value: formatCurrency(s.totalMarketValue), sub: `${diff >= 0 ? '增值' : '贬值'} ${formatCurrency(Math.abs(diff))} (${diff >= 0 ? '+' : ''}${pct}%)`, icon: <TrendingUp size={20} />, bg: diff >= 0 ? 'bg-success/15' : 'bg-error/15', color: diff >= 0 ? 'text-success' : 'text-error' }; })()] : []),
